...
float height(vec2 v){
    float h = 0;
	//vec2 center = vec2(3.0, 2.0);
	//float dist = length(v - center);
	//float falloff = exp(-dist * 1.7);
	//float noise = sin(0.5 * v.x) * pow(2, noiseOctave(v, 8)) * sin(0.5 * v.y);
	h = 0.15 * noiseOctave(v + vec2(0.2 * iTime, 0.1 * iTime), 3);
	//h = 0.75 * noise * falloff;
	if(h<0) h *= .5;
	return h * 2.;
}
...
vec3 shading_terrain(vec3 pos) {
	...
	float h = pos.z + .8;
	h = clamp(h, 0.0, 1.0);
	vec3 emissiveColor = mix(vec3(.2,.4,.7), vec3(.1,.6,.8), h);
